...
const limiter = rateLimit({
  ...
  max: 500, // limit each IP to 100 requests per windowMs ----- changed to 500
  message: {
    success: false,
...
app.use(favicon(path.join(__dirname, "../", "../", "favicono.svg")));
...
app.get("/", async (req, res) => {
  return res.json({
    response: "cryptosplashlive JSON REST API v.2.0.0",
